...
url_change="https://dweet.io/get/latest/dweet/for/manual_switch"
last_update = "default"
i = 0 # just to avoid to publish the first time (old values)
fail = 0
...
while True:
	...
	if js["this"] != "succeeded" and fail == 0 and i > 0:
		...
		print "change status request sent at " + str(ts)
		fail = 1
	else:
		if js["this"] == "succeeded":
			if i > 0 :
				fail = 0 # two consecutive failures never occur
				t = js["with"][0]["created"]
				if str(t) != str(last_update) :
					ts = int(time.time())
				 	senml = '{"bn": "Web-switch", "bt": "' + str(ts) + '", "e": [{"n": "Status", "u": "Boolean", "v": "change"}]}'
				 	change_pub.publish("web_switch/change_request", senml)
				 	print "change status request sent sent at " + str(ts)
					last_update = t
